...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectizeInput("countries", "Add Countries:", choices = c("NGA", "CMR", "TZA", "COD"), multiple = TRUE),
      conditionalPanel(
        condition = "input.countries.length > 0",
        selectizeInput("crop", "Select Crop:", choices = c("WHEA", "RICE", "MAIZ", "BARL"), multiple = TRUE),
        selectizeInput("output_type", "Select Output Type:", choices = c("PRODUCTIVITY", "YIELD"), multiple = TRUE)
      ),
      br(),
...
server <- function(input, output) {
  ...
  read_raster_from_drive <- function(folder_url, crop_chosen, output) {
    ...
    file_name <- ifelse(output == "PRODUCTIVITY", 
  ...
  extractData <- function(country, crop, output_type) {
    ...
    folder_url <- ifelse(output_type == "PRODUCTIVITY", "https://drive.google.com/drive/folders/1FqeBF-y4iMMHIxCEybXAlwqPJ7KSJkd5",
